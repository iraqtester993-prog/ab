...
const subscriptionTypes = reactive([
...
const areas = reactive(['المنصور', 'الكريعات', 'الدورة', 'اليرموك', 'الجامعة', 'أخرى'])

const towers = reactive([{ id: 1, name: 'برج الاتحاد', points: [] }])

const waTemplates = reactive([
...
const towerInfo = reactive({ name: 'برج الاتحاد', address: 'بغداد - المنصور', phone: '07700000000' })
let alertDays = 3
let nextId = 12
let finId = 6
let nextDebtId = 1
let lastArchiveMonth = ''

const users = reactive([
  { id: 1, name: 'المدير العام', username: 'admin', password: 'admin', lastLogin: null,
    permissions: {
      dashboard: true,
      subscribers: { view: true, add: true, edit: true, del: true, renew: true, settle: true },
...
function calcEndFromType(type, start) {
  ...
  const days = found ? found.days : 30
...
function autoArchive() {
  ...
  if (!lastArchiveMonth) { lastArchiveMonth = currentMonth; return }
  if (lastArchiveMonth !== currentMonth) {
    const prevMonth = lastArchiveMonth
    _store.finRecords.forEach(r => { if (r.date && r.date.startsWith(prevMonth) && !r.archived) r.archived = true })
    _store.subs.forEach(s => { if (s.status === 'expired' && s.end && s.end.startsWith(prevMonth) && !s.archived) s.archived = true })
...
function saveAllData() {
  const data = {
    subs: [..._store.subs], finRecords: [..._store.finRecords], archivedSubs: [..._store.archivedSubs],
    waTemplates, expenseCategories, towerInfo, alertDays, nextId, finId, nextDebtId, lastArchiveMonth,
    users, subscriptionTypes, areas, towers
  }
  ...
  try {
    ...
    if (saved) {
      ...
      _store.subs.splice(0, _store.subs.length, ...(data.subs || []).map(s => ({
        ...s, prevDebt: s.prevDebt || 0,
        debtHistory: (s.debtHistory || []).map(d => ({ ...d, remaining: d.remaining !== undefined ? d.remaining : d.amount, payments: d.payments || [] }))
      })))
      _store.finRecords.splice(0, _store.finRecords.length, ...(data.finRecords || []).map(r => { r.archived = r.archived || false; return r }))
      _store.archivedSubs.splice(0, _store.archivedSubs.length, ...(data.archivedSubs || []))
      _store.subs.forEach(s => { if (s.status === 'active' && s.end && s.end < todayStr()) s.status = 'expired' })
      if (data.waTemplates) { waTemplates.splice(0, waTemplates.length, ...data.waTemplates) }
      if (data.expenseCategories) { expenseCategories.splice(0, expenseCategories.length, ...data.expenseCategories) }
      if (data.towerInfo) { Object.assign(towerInfo, data.towerInfo) }
      if (data.alertDays != null) alertDays = data.alertDays
      if (data.nextId != null) nextId = data.nextId
      if (data.finId != null) finId = data.finId
      if (data.nextDebtId != null) nextDebtId = data.nextDebtId
      if (data.lastArchiveMonth != null) lastArchiveMonth = data.lastArchiveMonth
      if (data.users) { users.splice(0, users.length, ...data.users) }
      if (data.subscriptionTypes) { subscriptionTypes.splice(0, subscriptionTypes.length, ...data.subscriptionTypes) }
      if (data.areas) { areas.splice(0, areas.length, ...data.areas) }
      if (data.towers) { towers.splice(0, towers.length, ...data.towers.map(t => ({ ...t, points: Array.isArray(t.points) ? t.points : [] }))) }
      autoArchive()
      return true
    } else {
      initDefaultData()
...
function restoreSession() {
  try {
    ...
    if (saved) { const d = JSON.parse(saved); auth.user = users.find(u => u.id === d.id) || null }
  } catch(e) {}
}
...
export {
  _store, subscriptionTypes, areas, towers, waTemplates, expenseCategories, towerInfo, alertDays,
  nextId, finId, nextDebtId, lastArchiveMonth, users, auth,
  daysBetween, calcEndFromType, todayStr, formatMoney, calcTotalDebt, recalcPrevDebt,
  saveAllData, loadAllData, autoArchive, can, doLogin, doLogout, restoreSession, getCurrentUser
}
